...
Use platform actions for Outlit-hosted agents, automations, signals, destinations, and selected workspace settings. Use [Integration Routes](/api-reference/integrations) for connecting external data sources such as Slack, Stripe, HubSpot, PostHog, or support tools.
...
The current platform action set focuses on the Agents, Automations, and Settings areas of the Outlit platform.
...
| Destinations | List configured destinations, get one destination with masked configuration | Show Slack channel options, create Slack channel destination, update, enable, disable, archive |
| Settings | Get workspace, report, and notification settings; search Slack-backed setting options | Update default timezone, update report Slack channel, set default notification destination |
...
| `POST /api/destinations` | Create one Slack channel destination |
...
| `GET /api/settings/notifications` | Get notification settings and Slack-backed destinations |
| `GET /api/settings/notifications/options` | Show Slack channel options for notification settings |
| `POST /api/settings/notifications/default` | Set the default notification destination |
