...
`piToolNames` includes SQL, customer relationship and Attention reads, customer ownership and access actions, customer identity review and merge tools, and the broader Pi-supported Platform surface. The default membership comes directly from Core's `default` consumer policy and contains only the nine read-only customer intelligence tools above; import `analyticalToolNames` for that same policy plus `outlit_schema` and `outlit_query`. The broader Pi policy also includes the product-facing Feature list, create, archive, and customer-usage tools. Customer merge execution has no supported undo and requires a reviewed preview plus explicit `customer_identity:merge` permission. Execution retries must reuse the stable request ID and identical request inputs. The default and analytical policies remain unchanged.
