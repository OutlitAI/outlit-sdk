...
`allPublicToolNames` includes SQL. The default membership comes directly from Core's `default` consumer policy and excludes SQL; import `analyticalToolNames` for the Core-owned analytical policy. Integration setup is limited to safe browser handoff and status tools—provider credentials and configuration stay in the authored web UI.
