...
<Note>
  MCP registers the entire public catalog, including customer ownership/access and the safe destination, integration setup/status, activation, and workspace-settings capabilities described in [Platform Capabilities](/ai-integrations/platform-actions). Generic agent, automation, signal, identity-merge, notification-send, credential, and destructive integration operations are not public tools.
</Note>
