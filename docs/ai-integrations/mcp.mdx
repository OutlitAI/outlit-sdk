...
Read tools require `agents:read`. Write tools require `agents:write`. `outlit_destination_options` is non-mutating but requires `agents:write` because it lists Slack channel choices for destination creation. Destination tools return masked configuration only.
