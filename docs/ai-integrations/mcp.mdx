...
API-key authenticated MCP clients can also inspect agent and automation configuration through read-only platform action tools:
...
The only current write action in this platform action group is `outlit_agent_create_from_template`, which creates supported template resources in draft mode.
