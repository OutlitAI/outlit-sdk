...
| `outlit_destination_create` | Create one Slack channel destination |
...
| `outlit_settings_notifications_get` | Get notification settings and Slack-backed destinations |
| `outlit_settings_notifications_options` | Search Slack channel options for notification settings |
| `outlit_settings_notifications_default_set` | Set the default notification destination |

Read tools require `agents:read`. Write tools require `agents:write`. `outlit_destination_options` is non-mutating but requires `agents:write` because it lists Slack channel choices for destination creation. The settings option tools are read-scoped and accept optional `search` and `limit` inputs for Slack channel discovery. Destination tools return masked configuration only. Settings tools return projected settings DTOs, not raw workspace or destination database rows.
