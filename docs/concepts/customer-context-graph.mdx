...
The customer context graph is a structured representation of everything Outlit knows about your customers. It has three layers:
...
3. **Signals** — Activity from every source: pageviews from your website, events from your product, charges from Stripe, sessions from PostHog
...
|--------|-----------------|----------------|
| **Browser SDK** | Pageviews, form submissions, engagement time, UTM attribution | Add the Outlit script or npm package to your site |
| **Server SDK** | Backend events (API calls, exports, notifications) | Use the Node.js or Rust SDK |
| **Stripe** | Subscriptions, charges, billing status changes | Connect via Outlit dashboard |
| **PostHog** | Product usage sessions and feature engagement | Connect via Outlit dashboard |
...
Outlit's graph connects all of these. A single `outlit customers get acme.com --include timeline` shows the complete story: which pages Acme's team viewed before signing up, how they used the product after onboarding, and what their billing trajectory looks like.
