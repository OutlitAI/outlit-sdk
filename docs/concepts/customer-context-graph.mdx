...
The customer context graph is Outlit's unified data model. It connects every customer interaction — website visits, product usage, billing events, support tickets, calls, emails, and Slack conversations — into a structured graph of entities, relationships, timelines, and facts. Instead of customer data scattered across Stripe, PostHog, your app database, support tools, and email, the graph gives you and your AI agents one place to query the complete customer profile.
...
3. **Activity timeline** — A chronological log of every interaction from every source: pageviews, form submissions, Stripe charges, support tickets, Slack messages, email threads, calls, and calendar events
4. **Facts** — Temporal assertions about each customer that capture *what it means*, not just what happened: churn risk signals, expansion opportunities, support escalations, competitive evaluations, champion changes, and more
...
| **Granola** | Meeting notes and customer-call summaries | Connect via [dashboard](https://app.outlit.ai/integrations) |
| **Google Calendar** | Meetings, attendees, scheduling patterns | Connect via [dashboard](https://app.outlit.ai/integrations) |
| **Pylon** | Support tickets, escalations, and customer requests | Connect via [dashboard](https://app.outlit.ai/integrations) |
...
- **Contacts → Accounts**: Via organizational email domains, explicit `customerId` attribution, and connected source data
- **Accounts → Billing**: Via verified billing integrations such as Stripe
...
Outlit's graph connects all of these — and goes further with **facts**. Instead of just showing raw data, a query like `outlit customers get acme.com --include users,revenue,activity` plus `outlit facts list acme.com` shows the complete story *with reasoning*: which pages Acme's team viewed before signing up, how they used the product after onboarding, what support tickets or calls changed the relationship, what their billing trajectory looks like, and what it all means — churn risk signals, expansion opportunities, champion changes, and competitive context.
...
Every fact carries **temporal context**. Public fact results expose `occurredAt` for when the supporting signal happened or was detected, plus `status` and provenance fields such as `sourceType`, `sourceId`, `sourceOccurredAt`, `sourceQuote`, and `permalink` when available. Outlit uses source time, status, and evidence to help agents distinguish current signals from historical context; newer evidence can supersede older context, but facts should still be read with their source and status rather than as self-updating claims.
...
- **Company activation** (`activatedAt`) is a monotonic account milestone derived by Core from the configured exact ordinary product event
- **Account billing statuses** (None, Trialing, Paying, Churned) describe the commercial relationship

Some contact stages are inferred automatically from integrations. The Outlit browser SDK auto-detects Discovered and Signed Up based on the identifiers provided. Engaged and Inactive are derived from tracked product activity.

One configured exact ordinary product event activates eligible contacts and their resolved company. Core writes each subject's activation timestamp once, and contact/company materialization remains independent. SDKs and connected analytics sources continue emitting ordinary product events; they do not authoritatively mutate activation.
...
<AccordionGroup>
  ...
  <Accordion title="Can AI agents access the full graph?">
    Yes. The [MCP integration](/ai-integrations/mcp) exposes the graph to remote MCP clients such as Cursor, VS Code, and other supported environments. For coding agents that run local commands, install [Agent Skills](/ai-integrations/skills). Agents can list customers, get timelines, search context, inspect facts and sources, and run SQL queries.
  </Accordion>
