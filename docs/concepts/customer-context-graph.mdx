...
Every fact carries **temporal context**. `validFrom` records when the supporting signal happened or became true, and `validUntil` can be set when a fact should no longer be treated as current. Outlit uses those fields, source metadata, and fact status to make facts time-aware for agents; newer evidence can supersede older context, but facts should still be read with their source and status rather than as self-updating claims.
