...
Some stages are inferred automatically from integrations. The Outlit browser SDK auto-detects Discovered and Signed Up based on the identifiers provided. Engaged and Inactive are derived from tracked product activity. For Activated, choose the ordinary product event that represents your product's value milestone; Outlit Core derives activation when that event is received through `track()` or a verified product integration.
