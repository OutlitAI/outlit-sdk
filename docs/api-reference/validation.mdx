---
title: "Validate API Key"
description: "Verify that an Outlit API key can authenticate Platform API requests"
api: "POST /api/validate-api-key"
---

## Endpoint

```http
POST https://app.outlit.ai/api/validate-api-key
```

## Authentication

<ParamField header="Authorization" type="string" required>
  Outlit API key using the `Bearer ok_...` format.
</ParamField>

## Request Body

No request body is required when validating the API key from the `Authorization` header.

## Example

```bash
curl -X POST https://app.outlit.ai/api/validate-api-key \
  -H "Authorization: Bearer ok_your_api_key"
```

## Success Response

```json
{
  "valid": true,
  "organizationId": "org_123",
  "createdById": "user_123",
  "authorization": {
    "grants": ["customer_intelligence:read"]
  }
}
```

<ResponseField name="valid" type="boolean">
  Whether the supplied API key is active and accepted.
</ResponseField>

<ResponseField name="organizationId" type="string">
  Organization associated with the API key.
</ResponseField>

<ResponseField name="createdById" type="string | null">
  User that created the API key, when available.
</ResponseField>

<ResponseField name="authorization.grants" type="string[]">
  Current capabilities granted to this API key. A consumer must only offer commands whose
  required authority appears in this list.
</ResponseField>

## Error Responses

Invalid or disabled keys return `401`:

```json
{
  "valid": false,
  "error": "Invalid credentials"
}
```

If the credential store is temporarily unavailable, the endpoint returns `503` with the same
failure envelope. Retry that response; it does not mean the key is invalid or revoked.
