...
Track explicit activation milestones. Engaged and Inactive are derived automatically from tracked product activity — Engaged from qualifying activity on multiple distinct days in the configured rolling window, and Inactive after no qualifying activity for 30 days by default — but the ingest API continues to accept those values for older SDKs and direct API callers.
