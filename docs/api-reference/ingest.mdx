...
<ParamField body="userIdentity" type="object">
  ...
  | `customerDomain` | string | Public customer/account domain used for billing and attribution |
...
<ParamField body="customerDomain" type="string">
...
For server-side tracking, the `visitorId` field can be omitted. Identity is resolved from the event data, including customer-only attribution when `customerId` and/or `customerDomain` are provided:

- **Identify events**: Use `email` and/or `userId` fields, plus optional customer context
- **Custom events**: Include top-level `email`, `userId`, `customerId`, and/or `customerDomain` fields
