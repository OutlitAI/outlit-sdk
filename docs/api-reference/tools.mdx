...
For Slack sources, the response's `record` contains the root message, a chronological
page of replies, and `pageInfo` with `hasMore` and `nextCursor`. Omit `cursor` for the
first page.
