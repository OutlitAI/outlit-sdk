...
The package also exports `publicToolContracts`, `publicToolNames`, Core-owned `consumerToolPolicies`, `defaultToolNames`, `analyticalToolNames`, `cliToolNames`, `allPublicToolNames`, `toolGatewayTransport`, `ingestTransport`, and enum lists such as `customerSourceTypes`. `OPPORTUNITY` is the canonical CRM opportunity source type; `CRM` and `CRM_OPPORTUNITY` are accepted input aliases.
