...
| `outlit_list_sources` | List concrete source records such as calls, emails, calendar events, support tickets, opportunities, or Slack conversations |
...
For Slack sources, `sourceId` is Outlit's canonical root-message ID returned by source
listings, search results, and fact source references—not a Slack timestamp. Omit
`cursor` for the first page, which contains the newest replies. The response's
`record.replies` is chronological within each page; following
`record.pageInfo.nextCursor` walks backward to older replies.
