...
| `outlit_get_value_feature_workspace` | Read configured Value Features, exact historical usage, eligible sources, and ready, partial, or unavailable candidate discovery |
| `outlit_create_value_feature` | Create or idempotently return one Value Feature observed by one event rule |
| `outlit_archive_value_feature` | Archive a Value Feature using its opaque id and current revision when another active feature remains |
| `outlit_get_customer_feature_usage` | Read exact historical Value Feature usage for one authorized customer |
...
### Read Customer Feature Usage

```json
{
  "tool": "outlit_get_customer_feature_usage",
  "input": {
    "customer": "acme.com",
    "weeks": 12
  }
