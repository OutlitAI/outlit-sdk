...
Customer merge execution has no supported undo. Preview first and inspect the returned impact and warnings. Execution requires the reviewed preview token, a stable request ID, and explicit `customer_identity:merge` permission. The example below does not execute the merge.
