...
| `outlit_get_customer_relationship` | Get the bounded, evidence-backed relationship shown on a customer page |
...
| `outlit_list_attention_items` | List authorized open or resolved Attention items |
| `outlit_get_attention_item` | Get one authorized Attention item with its assessment, timeline, and evidence summaries |
| `outlit_get_customer_identity` | Inspect identity coverage and possible split records for one exact customer ID |
| `outlit_list_identity_merge_suggestions` | List saved customer-identity merge suggestions and review history |
| `outlit_reject_identity_merge_suggestion` | Reject one saved suggestion while keeping both customer records separate |
| `outlit_merge_customers` | Preview or explicitly execute a reviewed customer merge into the chosen survivor |
| `outlit_get_customer_merge_status` | Read the current status of an admitted customer merge operation |
...
### Inspect customer identity

Use the exact customer ID, not a name or domain. Candidate records are leads for review. They do not prove that two records represent the same customer.

```json
{
  "tool": "outlit_get_customer_identity",
  "input": {
    "customerId": "cust_123"
  }
}
...
```json
{
  "tool": "outlit_list_identity_merge_suggestions",
  "input": {
    "customerId": "cust_123",
    "status": "suggested",
    "limit": 20
...
### Reject an identity merge suggestion

Rejection requires identity review permission. It keeps both customer records separate.
...
```json
{
  ...
  "input": {
    ...
    "reviewNotes": "The records belong to different companies."
  }
}
```
...
Customer merge execution has no supported undo. Preview first and inspect the returned impact and warnings. Execution requires the reviewed preview token, a stable request ID, and explicit `customer_identity:merge` permission. Execution retries must reuse the stable request ID and identical request inputs. The example below does not execute the merge.

```json
{
  "tool": "outlit_merge_customers",
  "input": {
    "survivingCustomerId": "cust_123",
    "duplicateCustomerId": "cust_456",
    "dryRun": true
  }
}
```

### Read customer merge status

Queued or running status does not mean that the merge completed. Reading status does not retry, cancel, or undo the operation.

```json
{
  "tool": "outlit_get_customer_merge_status",
  "input": {
...
### Read Exact Source Content

Exact lookup can include a `contentPage` with bounded content segments. The default
page size is 10 segments, with a maximum of 100. Pass the returned
`contentPage.page.nextCursor` with the same source type and ID when
`contentPage.page.hasMore` is true. Check `contentPage.completeness` before treating
the content as complete.
...
Example response with complete content:
...
```json
{
  ...
  "summary": "The customer asked for a renewal proposal.",
  "permalink": null,
  "provider": "gong",
  ...
  "contentPage": {
    "sourceType": "CALL",
    "sourceId": "call_123",
    "contentKind": "call_transcript",
    "segments": [{ "id": "segment_1", "text": "Please send the renewal proposal." }],
    "page": { "returned": 1, "hasMore": false, "nextCursor": null },
    "completeness": { "status": "complete", "omittedFields": [] }
  }
}
```

### Follow Person Profile Evidence

When a fact or search result references a `PERSON_PROFILE` source, use its source ID
for exact lookup. Replace the example ID with the ID from that reference.
...
Feature usage is factual historical evidence. A feature can report complete or partial counts,
no matches in the requested window, or unavailable source evidence. Unavailable evidence is not
converted to zero. Archive is the only self-service lifecycle action in the MVP.
Creating a Feature also creates the supporting weekly event-count and active-days metrics inside
Outlit; those metrics are not separate public resources.
