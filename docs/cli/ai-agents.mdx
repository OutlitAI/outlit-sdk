...
`onboard` does not connect third-party integrations by itself. Use `outlit integrations setup <provider> --json`, then `outlit integrations status <provider> --json`. The CLI starts a browser handoff when one is available; credential entry and provider-specific configuration stay in the Outlit web app.
