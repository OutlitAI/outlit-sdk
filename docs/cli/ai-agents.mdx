...
<Tip>
  Manual remote MCP connections for users in multiple workspaces may need a workspace-specific URL such as
...
| Agent Skills | `outlit setup skills` | Installs [Outlit agent skills](/ai-integrations/skills) via package runner |
...
Without `--yes`, the CLI lists detected agents before configuring. [Agent skills](/ai-integrations/skills) are also installed automatically.
...
Installs Outlit [agent skills](/ai-integrations/skills) via your package runner. No API key required.
