...
| `OUTLIT_NO_UPDATE_NOTIFIER` | Disable automatic interactive CLI update notices when set | — |
...
| `update-check.json` | Cached latest CLI version and installer hint | Automatic update check |
...
```bash
OUTLIT_NO_UPDATE_NOTIFIER=1 outlit customers list
```
...
```bash
outlit users list --journey-stage INACTIVE --no-activity-in 30d --json \
