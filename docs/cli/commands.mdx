...
Create event-based Behavior Metrics using the same contract as the Outlit UI, API, and MCP tool.

```text
outlit metrics create --source-key <source-key> --event-name <event-name> --behavior-key <behavior-key> --label <label> [--property-filters <json-array>] [flags]
```
...
The source key, exact event name, stable lower-snake-case behavior key, and label are required. `--property-filters` accepts an optional JSON array of up to five typed `equals` or `exists` filters.
