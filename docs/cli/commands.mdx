...
outlit settings notifications get [flags]
outlit settings notifications options [--search <text>] [--limit <count>] [flags]
outlit settings notifications default set --destination-id <id> [flags]
```
...
Workspace settings are under `result.data.settings`. Report settings are under `result.data.report`. Notification settings are under `result.data.notifications`. The report and notification options commands return Slack connection status and bounded channel choices so agents can choose valid IDs before updating settings; use `--search` to filter channels by name or ID and `--limit` to bound results from 1 to 100. Read commands require `agents:read`; update and default-selection commands require `agents:write`.
