...
Create workspace usage metrics for configuration that can be referenced by Outlit workflows.

```text
outlit usage-metrics create --name <name> [--description <description>] [flags]
```
...
outlit usage-metrics create --name "Weekly Active Teams" --json
```

`--name` is required. `--description` is optional and blank values are omitted from the request.
