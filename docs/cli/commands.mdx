...
outlit agents create-custom [flags]
outlit agents update-profile <id> [flags]
outlit agents update-instructions <id> [flags]
outlit agents update-actions <id> [flags]
...
outlit agents update-instructions 10000000-0000-4000-8000-000000000004 --instructions "Prioritize recent escalations" --json
...
<Note>
  `create-from-template` creates supported template resources in draft mode. `update-actions` requires either `--action-keys` or explicit `--clear`. Custom agent write commands, lifecycle commands, and rename commands require an API key with `agents:write`.
</Note>
...
Automation create bodies must include `agentId`; update bodies must include `agentId` and `enabled`. The API maps that agent ID to the internal hosted-agent processor; callers do not provide raw processor JSON.
...
outlit destinations create-webhook [flags]
...
Destination list results are under `result.data.destinations`; get, create, update, enable, and disable results are under `result.data.destination`. `destinations update` requires either `--enabled` or `--disabled` so URL/name edits do not silently change lifecycle state. Archive results return `result.data.destination.id` and `result.data.destination.archivedAt`. Raw destination secrets and unmasked provider configuration are not returned.
