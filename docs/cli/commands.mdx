...
| Argument | Required | Description |
|----------|----------|-------------|
| `payload` | No | Notification payload as JSON or a raw string. Any JSON-serializable object, array, scalar, or string is accepted. |
...
<Note>
  `--payload-file` takes precedence over the positional payload. The CLI reads file contents first, then falls back to the positional payload when no file is provided.
</Note>
