...
Inspect, historically preview, and explicitly configure Core-derived company activation.
Company activation is a monotonic account milestone stored as `activatedAt`; it is
separate from each contact's journey stage.

Outlit evaluates one to three existing, non-archived `CUSTOMER`-grain signals for the
same company. SDKs and connected analytics sources continue sending ordinary product
events. They do not send a synthetic activation event or authoritatively mutate company
lifecycle state.
...
The command returns the saved definition and its signal metadata. The compatibility
block reports the existing contact-only activation event separately; Outlit never
silently migrates it into a company definition.

### Preview

Preview a candidate definition against historical signal occurrences:

```bash
# One signal: ANY is implied
outlit activation preview \
  --signal 10000000-0000-4000-8000-000000000001 \
  --lookback-days 30 \
  --json

# All selected signals for the same company within 30 days
outlit activation preview \
  --signals 10000000-0000-4000-8000-000000000001,10000000-0000-4000-8000-000000000002 \
  --match ALL \
  --window 30d \
  --lookback-days 60 \
  --example-limit 10 \
...
Preview is always read-only. It never saves the definition, changes `activatedAt`, or
materializes activation milestones.

| Flag | Required | Description |
|------|----------|-------------|
| `--signal` | One of signal/signals | One customer-grain signal UUID; implies `ANY` |
| `--signals` | One of signal/signals | Comma-separated one to three unique signal UUIDs |
| `--match` | For multiple signals | `ANY`, `ALL`, or `AT_LEAST` |
| `--threshold` | For `AT_LEAST` | Integer from 2 through the selected signal count |
| `--window` | No | `ALL`/`AT_LEAST` window, at most `168h` or `90d` |
...
`ANY` rejects threshold and window. A single-signal definition must use `ANY`. `ALL`
requires two or three signals and rejects threshold. `AT_LEAST` requires two or three
signals plus `--threshold`.

### Update or disable

After reviewing a preview, explicitly replace the complete definition:

```bash
outlit activation update \
  --signals 10000000-0000-4000-8000-000000000001,10000000-0000-4000-8000-000000000002,10000000-0000-4000-8000-000000000003 \
  --match AT_LEAST \
  --threshold 2 \
  --window 168h \
  --json
...
The referenced customer-grain signals must already exist. Activation commands do not
create signals; create them with `outlit signals create` or select existing signals.

To stop future company activation evaluation:

```bash
outlit activation disable --json
```

Disabling preserves every existing `activatedAt` value and canonical activation
milestone because company activation is monotonic.
