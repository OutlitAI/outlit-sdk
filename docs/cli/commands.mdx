...
After reviewing a preview, explicitly save the complete definition:

```bash
outlit activation set \
...
`--disable` cannot be combined with definition flags. Disabling preserves every existing
`activatedAt` value and canonical activation milestone because company activation is
monotonic.

All three commands preserve Core's standard structured command envelope in `--json` and
