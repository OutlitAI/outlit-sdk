...
All four commands preserve Core's standard structured command envelope in `--json` and
piped output. Local validation errors use the CLI's existing `missing_input` or
`invalid_input` error shape; authenticated API errors retain the standard Core error
envelope.
...
Use `sources list` for deterministic source enumeration and `sources get` for exact lookup.
...
| `--customer` | string | — | Customer UUID, domain, or name |
| `--participant` | string | — | Participant email or name |
| `--provider` | string | — | Provider identifier such as `gmail`, `gong`, or `google-calendar` |
| `--has-transcript` | boolean | `false` | Only list call sources with a finalized transcript |
| `--after` | string | — | Filter to sources occurring after this ISO 8601 datetime |
| `--before` | string | — | Filter to sources occurring before this ISO 8601 datetime |
...
| `--limit` | integer | 50 | Slack replies per page (1–100). Ignored for other source types. |
| `--cursor` | string | — | Opaque Slack reply cursor from `record.pageInfo.nextCursor` |

#### Slack list → get

```bash
# Find the canonical sourceId
outlit sources list --customer acme.com --source-type SLACK --json

# Open that Slack thread
outlit sources get --source-type SLACK --source-id root_123 --limit 25 --json

# Continue when record.pageInfo.hasMore is true
NEXT_CURSOR="..."
outlit sources get --source-type SLACK --source-id root_123 --cursor "$NEXT_CURSOR" --json
```

For Slack, `sourceId` is Outlit's canonical root-message ID returned by source
listings, search results, and fact source references—not a Slack timestamp. The first
page contains the newest replies, while `record.replies` is chronological within each
page. Following `record.pageInfo.nextCursor` walks backward to older replies.
