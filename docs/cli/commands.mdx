...
Create event-based Behavior Metrics using the same contract as the Outlit UI, API, and MCP tools. Discover an eligible source, then its attributed events, before creating a metric.
...
`metrics create` returns an enabled, production Behavior Metric immediately. Source readiness remains independent ingestion-health information.
