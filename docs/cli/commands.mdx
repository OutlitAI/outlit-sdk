...
outlit sources get --source-type SLACK --source-id root_123 --cursor <next-cursor>
outlit sources get --source-type SUPPORT_TICKET --source-id ticket_456 --json
```

Slack exact-source responses include a structured `record.root`, chronological
`record.replies`, and `record.pageInfo` with `hasMore` and `nextCursor`.
