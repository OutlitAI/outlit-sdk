...
<Note>
  `create-from-template` creates supported template resources in draft mode. Listing agents, templates, and actions is read-only.
</Note>
...
Automation commands return command envelopes. List results are under `result.data.automations`; get results are under `result.data.automation`.

## Signals

List configured automation signals.
...
Signal list results are under `result.data.signals`.

## Destinations

List configured automation destinations with masked configuration only.
...
Destination list results are under `result.data.destinations`. Raw destination secrets and unmasked provider configuration are not returned.
