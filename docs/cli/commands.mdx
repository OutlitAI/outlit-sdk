  ...
  "items": [
    {
      ...
      "sourceMetadata": {
        ...
        "permalink": "https://app.outlit.com/calls/call_123"
      }
    }
  ],
  "pagination": {
...
<Note>
  The `sourceMetadata` object provides provenance for each fact. All fields are nullable — `null` indicates the source information is unavailable.
</Note>

## Search

Semantic search across customer context — facts, emails, transcripts, and signals. Supports natural language queries, direct source lookups, and document type filtering.
...
| Argument | Required | Description |
|----------|----------|-------------|
| `query` | No* | Natural language search query. *Required unless `--source-type` and `--source-id` are both provided |
...
| `--doc-types` | string | — | Comma-separated document types: `fact`, `email_chunk`, `transcript_chunk` |
| `--source-types` | string | — | Comma-separated broad source type filter |
| `--source-type` | string | — | Exact source type for direct lookup (must pair with `--source-id`) |
| `--source-id` | string | — | Exact source ID for direct lookup (must pair with `--source-type`) |

<Note>
  **Validation rules:**
  - Either `query` or both `--source-type` and `--source-id` must be provided
  - `--source-type` and `--source-id` must always be used together
  - `--source-types` cannot be combined with `--source-type`/`--source-id`
</Note>
