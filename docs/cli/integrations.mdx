...
`outlit integrations status`. Both use the public tool gateway. Setup requires an API key with
`integrations:manage` authority.
...
Use `--config-stdin` to opt into reading exactly one strict JSON object from stdin:

```json
{
  "connectionMode": "provider-specific-mode",
  "credentials": {
    "apiKey": "<provider credential>"
  },
  "configuration": {
    "kind": "provider-specific configuration"
  }
}
```
