...
Connect a new integration. This is the legacy direct-connect command; use `integrations setup` for new agent flows because it exposes provider capabilities and follow-up steps.
...
Browser/Nango providers return a `connectUrl` and `sessionId` in JSON mode. Show the URL/code to the user when browser approval is required, then poll with:
