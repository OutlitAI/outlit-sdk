...
<Info>
  The `discovered` and `signed_up` stages are automatically inferred from identify calls. Engaged and Inactive are derived automatically from tracked product activity: Engaged requires qualifying activity on multiple distinct days in the configured rolling window, and Inactive is applied after no qualifying activity for 30 days by default. Use `track()` for product events instead of calling deprecated `engaged()`/`inactive()` stage methods. See [Customer Journey](/concepts/customer-journey#automatic-engagement-and-inactivity) for the current rules.
</Info>
