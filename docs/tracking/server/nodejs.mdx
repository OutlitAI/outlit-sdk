---
title: "Node.js SDK"
description: "Server-side tracking with identity resolution"
---

## Overview

The Node.js SDK is designed for server-side tracking. It supports:

- **Identified tracking** - Use `email` or `userId` for immediate resolution
- **Anonymous tracking** - Use `fingerprint` for events before user identification
- **Automatic backfill** - Anonymous events are linked when identity is established
- **Efficient batching** - Events are queued and sent in batches

<Info>
  Use server-side tracking for events that happen in your backend: subscription changes, feature usage from APIs, background jobs, desktop apps, etc.
</Info>
...
// Track with fingerprint (anonymous, linked later)
outlit.track({
...
<ParamField path="fingerprint" type="string">
  Device identifier for anonymous tracking. Events are linked to a user when `identify()` is called with the same fingerprint.
</ParamField>
...
<Info>
  At least one of `fingerprint`, `email`, or `userId` is required. Use `fingerprint` for anonymous tracking, `email` for immediate resolution.
</Info>

### `outlit.identify(options)`

Identify a user and optionally link anonymous events.

```typescript
outlit.identify({
  email: 'jane@acme.com',
  userId: 'usr_12345',
  fingerprint: 'device_abc123', // Links this device's events to the user
  traits: {
    name: 'Jane Doe',
    company: 'Acme Inc',
    plan: 'enterprise'
...
<ParamField path="fingerprint" type="string">
  Device identifier. If provided, all events tracked with this fingerprint are retroactively linked to the user.
</ParamField>
...
<Tip>
  Use `fingerprint` to link anonymous events to a user. When a user signs in, call `identify()` with both their email and the device fingerprint to backfill their anonymous activity.
</Tip>
