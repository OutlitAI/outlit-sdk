...
<Info>
  For anonymous tracking before user sign-in, use `fingerprint`. See [Device Tracking](/tracking/identity/anonymous-visitors#device-tracking-non-browser) for details.
</Info>
...
<CodeGroup>
```bash cargo
cargo add outlit tokio --features tokio/full
```
    ...
    tauri::Builder::default()
        ...
        .run(|app, event| {
            if let tauri::RunEvent::Exit = event {
                // Flush on exit
                let client = app.state::<Arc<Outlit>>();
                futures::executor::block_on(client.shutdown()).ok();
            }
        });
}
```
...
<CardGroup cols={2}>
  <Card title="Device Tracking" icon="mobile" href="/tracking/identity/anonymous-visitors#device-tracking-non-browser">
