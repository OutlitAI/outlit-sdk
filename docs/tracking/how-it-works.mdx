...
Outlit builds a [customer context graph](/concepts/customer-context-graph) from every customer interaction — website visits, product usage, billing, support, conversations, and more. Here's how raw source data becomes structured customer context.
...
Visitors link to contacts via identity resolution. Contacts belong to accounts via email domain matching. All activity flows into unified customer profiles and timelines per contact and account.
