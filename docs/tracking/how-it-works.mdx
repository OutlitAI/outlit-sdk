---
title: "How It Works"
description: "How Outlit connects customer systems, resolves identities, structures customer context, and makes it queryable"
---
...
Every fact carries **temporal context** — a `validFrom` date, source occurrence metadata when available, fact status, and optional expiration. `validFrom` tells agents when the supporting signal happened or became true. `validUntil` is available when a fact should stop being treated as current, but facts should be read with source time, status, and evidence rather than as self-updating claims.
...
# SQL
outlit sql "SELECT domain, mrr_cents FROM customer_dimensions WHERE billing_status = 'PAYING'"
```
...
For custom integrations, the [Ingest API](/api-reference/ingest) accepts events from any source, and the [public tool gateway](/api-reference/tools) exposes customer profiles, timelines, facts, sources, search, and SQL programmatically.
