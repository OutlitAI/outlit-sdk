...
```typescript
outlit.track('onboarding_completed', { flow: 'onboarding', step: 'completed' })
```

<Info>
  Configure this exact event as your activation event. Outlit Core applies its first match to eligible contacts and their resolved company. Billing status comes from verified integrations such as Stripe. See [Customer Journey](/concepts/customer-journey).
</Info>
