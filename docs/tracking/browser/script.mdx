...
window.outlit.track('onboarding_completed', { flow: 'onboarding' })
```

<Info>
  Configure this exact event as your activation event. For each eligible contact and resolved company independently, Outlit Core sets `activatedAt` on the first match only when that subject's timestamp is null. Billing status comes from verified integrations such as Stripe. See [Customer Journey](/concepts/customer-journey).
</Info>
