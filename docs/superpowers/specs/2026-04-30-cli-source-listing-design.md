...
- Normalize `participants` in list output for every source type.
...
Each item should use a compact normalized shape:
...
```ts
occurredAt desc, sourceType asc, id desc
```

The cursor should encode the last returned tuple:

```ts
{ occurredAt: string; sourceType: string; id: string }
```
