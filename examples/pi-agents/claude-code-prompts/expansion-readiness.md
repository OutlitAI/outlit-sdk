...
3. Gather customer details, revenue, users, timeline, behavior metrics, facts, semantic search evidence, and source evidence.
   Use fact filters such as `outlit facts list <customer> --fact-types EXPANSION,PRODUCT_USAGE,REQUIREMENTS,SENTIMENT --fact-categories MEMORY --json` for buying intent, plan-limit pain, premium requests, and value realization evidence.
