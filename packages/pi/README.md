...
SQL and the other Pi-supported Platform capabilities are available from `@outlit/tools`, but the default Pi policy intentionally stays focused on customer intelligence. A custom Pi extension can opt into every Pi-supported tool with `piToolNames`, including customer relationship and Attention reads, customer ownership and access actions, customer identity review and merge tools, product-facing Feature list, create, archive, and customer-usage tools, plus safe integration setup, destination, activation, and workspace-settings actions. Customer merge execution has no supported undo and requires a reviewed preview plus explicit `customer_identity:merge` permission. Execution retries must reuse the stable request ID and identical request inputs. The default and analytical policies remain unchanged.
