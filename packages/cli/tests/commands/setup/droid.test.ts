...
describe("setup droid", () => {
  ...
  test("installs the outlit skill for Droid", async () => {
    const writeSpy = spyOn(process.stdout, "write").mockImplementation(() => true)
    const { default: droidCmd } = await import("../../../src/commands/setup/droid")

    try {
      await droidCmd.run!({
        args: { json: true },
      } as Parameters<NonNullable<typeof droidCmd.run>>[0])
    } finally {
      ...
      expect(result.agent).toBe("droid")
