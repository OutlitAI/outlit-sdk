...
describe("setup opencode", () => {
  ...
  test("installs the outlit skill for OpenCode", async () => {
    const writeSpy = spyOn(process.stdout, "write").mockImplementation(() => true)
    const { default: opencodeCmd } = await import("../../../src/commands/setup/opencode")

    try {
      await opencodeCmd.run!({
        args: { json: true },
      } as Parameters<NonNullable<typeof opencodeCmd.run>>[0])
    } finally {
