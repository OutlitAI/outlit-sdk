...
describe("setup gemini", () => {
  ...
  test("installs the outlit skill for Gemini CLI via gemini-cli agent mapping", async () => {
  ...
  test("no auth required — command has no authArgs", async () => {
    const { default: geminiCmd } = await import("../../../src/commands/setup/gemini")
    expect(geminiCmd.args).toBeDefined()
    const argKeys = Object.keys(geminiCmd.args!)
