...
describe("completions command", () => {
  ...
  test("bash — subcommand dispatch", async () => {
    ...
    expect(out).toContain(`[[ $COMP_CWORD -eq 2 && "${bashCompWord(1)}" == "usage-metrics" ]]`)
  ...
  test("zsh — top-level commands", async () => {
    ...
    expect(out).toContain("'usage-metrics:Configure workspace usage metrics'")
  ...
  test("zsh — subcommand dispatch", async () => {
    ...
    expect(out).toContain("'setup:Start a safe browser setup handoff'")
  ...
  test("fish — flag completions with nested commands", async () => {
    ...
    expect(out).not.toContain("-n '__outlit_using_cmd integrations setup' -l config")
    ...
    expect(out).toContain("-n '__outlit_using_cmd usage-metrics create' -l name")
    expect(out).toContain("-n '__outlit_using_cmd usage-metrics create' -l description")
